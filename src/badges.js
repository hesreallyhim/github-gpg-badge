...
export const generateSplitBadge = (hasKey) => {
  const labelWidth = 90;
...
export const generateFlatBadge = (hasKey, square = false) => {
...
export const generateForTheBadge = (hasKey) => {
  const statusText = hasKey ? 'AVAILABLE' : 'NONE';
  const rightWidth = hasKey ? 90 : 55;
  const totalWidth = 80 + rightWidth;
  const rightColor = hasKey ? '#22c55e' : '#6b7280';

  return `<svg width="${totalWidth}" height="28" viewBox="0 0 ${totalWidth} 28" xmlns="http://www.w3.org/2000/svg">
  <rect x="0" y="0" width="80" height="28" fill="#555"/>
  <rect x="80" y="0" width="${rightWidth}" height="28" fill="${rightColor}"/>
  <g fill="#010101" fill-opacity="0.3">
    <text x="40" y="19" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11" font-weight="bold" textLength="60" lengthAdjust="spacing">GPG KEY</text>
    <text x="${80 + rightWidth/2}" y="19" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11" font-weight="bold">${statusText}</text>
  </g>
  <g fill="white">
    <text x="40" y="18" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11" font-weight="bold" textLength="60" lengthAdjust="spacing">GPG KEY</text>
    <text x="${80 + rightWidth/2}" y="18" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11" font-weight="bold">${statusText}</text>
  </g>
</svg>`;
};

export const generateErrorBadge = (message = 'error') => {
...
export function generateBadge(hasKey, style, options) {
  ...
  switch (style) {
    ...
    case 'for-the-badge':
      return generateForTheBadge(hasKey);
    case 'split':
    default:
...
export const VALID_STYLES = ['split', 'card', 'flat', 'flat-square', 'for-the-badge'];
